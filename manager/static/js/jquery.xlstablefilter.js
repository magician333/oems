  ...
  $.widget("ui.xlsTableFilter", {
    ...
    _create: function() {
      ...
      $.each(headers, function(i, h) {
        var filterable =
          (self.options.onlyColumns == null &&
            jQuery.inArray(i, self.options.ignoreColumns) == -1) ||
          jQuery.inArray(i, self.options.onlyColumns) !== -1
            ? true
            : false;
        if (filterable == true) {
          $(this)
            .addClass("xlsFilterHeader")
            .click(function(e) {
    ...
    _openFilter: function(header) {
      ...
      var filterDiv = this._createDialogDiv(
        "divXlsFilter",
        "筛选 - " + header.text(),
        filterContent
      );
    ...
    _getFilterContent: function(header) {
      ...
      $.each(
        $(self.element).find("tr td:nth-of-type(" + colNum + ")"),
        function(i, el) {
      ...
      filter =
        filter +
        '<div>文本搜索 <input type="text" id="filterSearch" style="width: 200px;"></div>';
      filter =
        filter +
        '<div id="xlsFilterAllNone"><span id="xlsFilterAll">全选</span> | <span id="xlsFilterNone">取消全选</span></div>';
    ...
    _addFilterRow: function(val, colNum) {
      ...
      if (this.filters[this.elid]["col" + colNum]) {
        var checked =
          jQuery.inArray(val, this.filters[this.elid]["col" + colNum]) == -1
            ? false
            : true;
      } else {
        var checked = true;
      ...
      var row =
        '<div class="xlsFilterRow"><input type="checkbox" style="' +
        (this.options.checkStyle == "default" ? "" : "display: none;") +
        '" value="' +
        val +
        '" ' +
        ...
        (this.options.checkStyle == "default"
          ? ""
          : '<div class="xlsFilterCheck xlsFilterCheckO' +
            (checked == true ? "n" : "ff") +
            '"></div>') +
        "<span>" +
        text +
    ...
    checkRow: function(row, check) {
      ...
      if (this.options.checkStyle == "custom") {
        ...
        checkdiv.addClass(
          check == true ? "xlsFilterCheckOn" : "xlsFilterCheckOff"
        );
        checkdiv.removeClass(
          check == true ? "xlsFilterCheckOff" : "xlsFilterCheckOn"
        );
      }
    },
    ...
    _filterTable: function(colNum) {
      ...
      $.each($("div.xlsFilterRow input:checked"), function() {
        self.filters[self.elid][filterRow][
          self.filters[self.elid][filterRow].length
        ] = $(this).val();
      });

      if ($("div.xlsFilterRow input").not(":checked").length === 0) {
        self.element
          .find("thead th:nth-of-type(" + colNum + ")")
          .removeClass("xlsFilteredColumn");
      } else {
        self.element
          .find("thead th:nth-of-type(" + colNum + ")")
          .addClass("xlsFilteredColumn");
      }
        ...
        $.each(self.filters[self.elid], function(key, val) {
          ...
          if (visible == true) {
            if (
              jQuery.inArray(
                self._adjustCase(
                  $row.children("td:nth-of-type(" + i + ")").text()
                ),
                val
              ) == -1
            ) {
    ...
    _printRowsDisplayed: function() {
      if (this.options.rowsDisplay !== false) {
        ...
        var rowDisplay =
          typeof this.options.rowsDisplay == "object"
            ? this.options.rowsDisplay
            : $("#" + this.options.rowsDisplay);
        rowDisplay.html(
          "Displaying " +
            VisibleCount +
